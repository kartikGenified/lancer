...
const RedeemGifts = ({navigation,route}) => {
  ...
  console.log("Scheme ID",route.params)
  ...
  useEffect(() => {
    if (userPointData) {
      ...
      {
        route.params?.schemeType == "yearly" ? setPointBalance(userPointData.body.point_balance) : setPointBalance(route.params?.pointBalance)
      }
    ...
    <View
      ...
      <View
        ...
        <TouchableOpacity onPress={()=>{
          if(cart.length!==0)
          {
            navigation.replace("CartList",{cart:cart,schemeType:schemeType,schemeID:schemeID,schemeGiftCatalogue:schemeGiftCatalogue})
          }
          else (
            setError(true),
